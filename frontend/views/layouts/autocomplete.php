...
<div class="service_autocomplete_search" style="">
	...
	<?php echo $form->field(new \common\models\CcServicesSearch, 'name', [				
				'options' => ['class'=>''],
				'addon' => [
					//'prepend' => ['content'=>'Usluge'],
					'append' => [
			            'content' => Html::submitButton('<i class="fa fa-search"></i>', ['class'=>'btn btn-default‚btn-lg']), 
			            'asButton' => true
			        ],
				]
			    ...
			    'pluginEvents' => [
				    ...
				    "typeahead:select" => "function(event, data) {
				    		if(data.id){
				    			$('#ccservicessearch-id').val(data.id); 
				    		} else if(data.industry_id){
				    			$('#ccsservicessearch-industry_id').val(data.industry_id); 
				    		} else if(data.action_id){
				    			$('#ccservicessearch-action_id').val(data.action_id); 
				    		} else if(data.object_id){
				    			$('#ccservicessearch-object_id').val(data.object_id); 
				    		}
				    		$('#autocomplete-search-form').submit();						    		
				    	}",
				]
			])->label(false) ?>
    <?= Html::activeHiddenInput(new \common\models\CcServicesSearch, 'id')?>
    <?= Html::activeHiddenInput(new \common\models\CcServicesSearch, 'industry_id')?>
    <?= Html::activeHiddenInput(new \common\models\CcServicesSearch, 'action_id')?>
    <?= Html::activeHiddenInput(new \common\models\CcServicesSearch, 'object_id')?>
    <?php /* Html::activeHiddenInput(new \common\models\CcServicesSearch, 'tag_id') */ ?>
	<?php ActiveForm::end(); ?>
</div>