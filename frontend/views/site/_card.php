...
<div class="card_container record-sm grid-item fadeInUp animated" id="card_container" style="">    
    ...
    <div class="primary-context normal">
        <div class="head major"><?= Html::a(c($model->name), Url::to('/s/'.slug($model->name)), ['class'=>'']) ?></div>
        <div class="subhead"><?= c($model->object->name) ?>
    ...
    <div class="secondary-context tease">
        ...
        <p><?php /* c($model->industry->name) */ ?></p>
    </div>
    <div class="action-area">
		<div class="button float-right">
        <?php if($state==null or $state==''): ?>
            <?php if($model->object->models): ?>
    			<?= Html::a('<i class="fa fa-shopping-cart"></i>&nbsp;'.Yii::t('app', 'Poruči'), Url::to('/order/'.slug($model->name)), ['class'=>'btn btn-info order_service', 'style'=>'color:#fff;', 'data-toggle'=>'modal', 'data-backdrop'=>false,  'data-target'=>'#object-models-order-modal'.$model->id]) ?>
        ...
        <?php else: ?>
            <?php if($model->object->models): ?>
                <?= $state=='order' ? Html::a('<i class="fa fa-shopping-cart"></i>&nbsp;'.Yii::t('app', 'Poruči'), Url::to('/order/'.slug($model->name)), ['class'=>'btn btn-info order_service', 'style'=>'color:#fff;', 'data-toggle'=>'modal', 'data-backdrop'=>false,  'data-target'=>'#object-models-order-modal'.$model->id]) : null ?>
